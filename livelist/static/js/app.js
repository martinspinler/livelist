...
function initApplication() {
    ...
    function livelist_update(msg) {
        ...
        // Song count stat
        const songCount = (msg.items || []).filter(i => i.song_id != null).length;
        const statsEl = document.getElementById("livelist-stats");
        if (statsEl) statsEl.textContent = songCount + (songCount === 1 ? " song" : " songs");
    ...
    /** Scroll the list to the current anchor. If the anchor is a song hidden
        inside a collapsed set, expand that set first so it becomes visible. */
    function scrollAnchorIntoView() {
        const ai = state.anchorItem;
        if (!ai) return;

        if (ai.classList.contains('livelist-item') && ai.classList.contains('d-none')) {
            let b = ai.previousElementSibling;
            while (b && !b.classList.contains('livelist-break')) b = b.previousElementSibling;
            if (b) {
                const setKey = b.dataset.setKey || parseInt(b.dataset.itemId);
                if (state.collapsedSets.has(setKey)) toggleSetCollapse(setKey);
            }
        }
        requestAnimationFrame(() => ai.scrollIntoView({block: 'center', behavior: 'smooth'}));
    }
    ...
    /** Collect keys for every rendered set ("__set1__" for the virtual first set,
        numeric item id for real break separators). */
    function collectSetKeys() {
        const keys = [];
        document.querySelectorAll('#livelist-items .livelist-break').forEach(b => {
            if (b.dataset.setKey) keys.push(b.dataset.setKey);
            else if (b.dataset.itemId) keys.push(parseInt(b.dataset.itemId));
        });
        return keys;
    }

    function allSetsCollapsed() {
        const keys = collectSetKeys();
        return keys.length > 0 && keys.every(k => state.collapsedSets.has(k));
    }

    /** Collapse or expand every set at once. */
    function collapseAllSets(collapse) {
        collectSetKeys().forEach(k => {
            const isCollapsed = state.collapsedSets.has(k);
            if (collapse && !isCollapsed) toggleSetCollapse(k);
            else if (!collapse && isCollapsed) toggleSetCollapse(k);
        });
        updateCollapseAllButton();
    }

    /** Toggle the overflow-menu button between "Collapse all" and "Expand all". */
    function updateCollapseAllButton() {
        const btn = document.getElementById('collapse-all-btn');
        if (!btn) return;
        const collapsed = allSetsCollapsed();
        btn.classList.toggle('bi-arrows-collapse', !collapsed);
        btn.classList.toggle('bi-arrows-expand', collapsed);
        btn.textContent = collapsed ? btn.dataset.labelExpand : btn.dataset.labelCollapse;
    }
    ...
    function setupSocketCallbacks() {
        socket.on("connect", () => {
            ...
            if (el) { el.className = 'conn-dot conn-on'; el.title = 'Connected'; }
        });
        socket.on("disconnect", () => {
            const el = document.getElementById('conn-status');
            if (el) { el.className = 'conn-dot conn-off'; el.title = 'Disconnected — not receiving updates'; }
        });
    ...
    function setupEventListeners() {
        ...
        document.getElementById('collapse-all-btn')?.addEventListener('click', () => {
            // If any set is expanded -> collapse all; otherwise expand all.
            collapseAllSets(!allSetsCollapsed());
        });

        document.getElementById('anchor-info')?.addEventListener('click', scrollAnchorIntoView);
